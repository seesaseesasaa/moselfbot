...
COMMON_WORDS = [
    ...
    'rings', 'kings', 'things', 'wings', 'songs', 'longs', 'wrongs', 'strings', 'springs', 'brings', 'swings', 'stings', 'clings', 'flings', 'slings', 'pings', 'dings', 'sings', 'tings', 'zings', 'bin[...]

    # ===== MPS (jumps, pumps, lumps, camps, temps, symptoms, attempts) =====
    'jumps', 'pumps', 'lumps', 'bumps', 'humps', 'rumps', 'stumps', 'thumps', 'clumps', 'grumps', 'plumps', 'frumps', 'slumps', 'trumps', 'camps', 'damps', 'lamps', 'ramps', 'stamps', 'clamps', 'tramp[...]

    # ===== RTS (parts, starts, hearts, arts, sports, courts, shorts, ports) =====
    'parts', 'starts', 'hearts', 'arts', 'sorts', 'sports', 'courts', 'shorts', 'snorts', 'ports', 'forts', 'carts', 'darts', 'farts', 'tarts', 'warts', 'charts', 'smarts', 'departs', 'imparts', 'upst[...]

    # ===== IES (babies, ladies, stories, worries, berries, carries, fairies, duties) =====
    'babies', 'ladies', 'copies', 'stories', 'worries', 'hurries', 'berries', 'cherries', 'fairies', 'varies', 'carries', 'marries', 'tarries', 'parties', 'calories', 'studies', 'duties', 'varieties',[...]

    # ===== NTS (wants, grants, plants, ants, events, moments, contents) =====
    'wants', 'grants', 'plants', 'ants', 'pants', 'chants', 'slants', 'stunts', 'blunts', 'counts', 'mounts', 'amounts', 'fronts', 'vents', 'dents', 'rents', 'tents', 'bents', 'scents', 'events', 'pre[...]

    # ===== NDS (hands, lands, bands, sounds, pounds, minds, finds, friends) =====
    'hands', 'lands', 'bands', 'sands', 'stands', 'brands', 'strands', 'grands', 'commands', 'demands', 'understands', 'wands', 'bonds', 'fonds', 'ponds', 'sounds', 'pounds', 'wounds', 'rounds', 'grou[...]

    # ===== RST (first, worst, burst, thirst, forest, breast, coast, fast) =====
    'first', 'worst', 'burst', 'thirst', 'cursed', 'nursed', 'pursed', 'dispersed', 'forest', 'breast', 'feast', 'beast', 'yeast', 'east', 'coast', 'roast', 'toast', 'boast', 'blast', 'fast', 'last', [...]

    # ===== ONG (long, song, strong, wrong, along, belong, throng, oblong) =====
    'long', 'song', 'strong', 'wrong', 'dong', 'gong', 'kong', 'pong', 'tong', 'along', 'among', 'belong', 'prolong', 'throng', 'oblong', 'sarong', 'lifelong', 'headlong', 'daylong', 'alllong', 'evlon[...]

    # ===== ALI (quality, reality, alien, alive, align, alliance, allow) =====
    'quality', 'reality', 'mortality', 'polarity', 'clarity', 'charity', 'scarlet', 'palette', 'mallet', 'ballet', 'wallet', 'pallet', 'bullet', 'mullet', 'cullet', 'gullet', 'alien', 'alienate', 'ali[...]

    # ===== ERS (players, makers, fighters, teachers, watchers, factors, masters) =====
    'players', 'makers', 'fighters', 'miners', 'speakers', 'teachers', 'preaches', 'breaches', 'teaches', 'reaches', 'beaches', 'peaches', 'leaches', 'watchers', 'catchers', 'pitchers', 'batchers', 'h[...]

    # ===== EXC (except, exception, excel, excuse, execute, excite, exclude) =====
    'except', 'exception', 'excess', 'exchange', 'excite', 'excitement', 'excel', 'excellent', 'exclaim', 'exclude', 'exclusive', 'excuse', 'execute', 'execution', 'executive', 'exemplify', 'exempt', [...]

    # ===== ENT (went, rent, event, prevent, content, moment, element) =====
    'went', 'rent', 'dent', 'bent', 'sent', 'tent', 'event', 'prevent', 'content', 'intent', 'extent', 'consent', 'dissent', 'present', 'represent', 'resent', 'absent', 'accent', 'ascent', 'descent', [...]

    # ===== EAD (dead, read, head, bread, thread, spread, steady, already) =====
    'dead', 'read', 'head', 'bread', 'thread', 'spread', 'dread', 'tread', 'instead', 'bead', 'lead', 'mead', 'stead', 'steady', 'already', 'shred', 'threadbare', 'unstead', 'misled', 'mislead', 'unre[...]
    ...
    'sky', 'spy', 'fly', 'guy', 'buy', 'shy', 'icy', 'ivy', 'any', 'vex', 'hex', 'sex', 'nex', 'yep', 'wax', 'tax', 'fax', 'sax', 'pax', 'lax', 'max', 'zap', 'rap', 'map', 'nap', 'cap', 'gap', 'lap', [...]

    # ===== ALL -ING WORDS =====
    'sing', 'ring', 'wing', 'king', 'ping', 'ding', 'ting', 'zing', 'bing', 'jing', 'ling', 'ming', 'ning', 'ving', 'bring', 'cling', 'fling', 'sling', 'sting', 'string', 'swing', 'thing', 'wring', 's[...]

    # ===== ALL -TION/-SION WORDS =====
    'action', 'faction', 'traction', 'fraction', 'reaction', 'creation', 'vacation', 'location', 'notation', 'rotation', 'citation', 'dictation', 'nation', 'station', 'ration', 'potion', 'lotion', 'mo[...]

    # ===== ADDITIONAL LONG-TAIL WORDS =====
    'light', 'night', 'right', 'sight', 'tight', 'fight', 'might', 'flight', 'plight', 'slight', 'height', 'weight', 'eight', 'high', 'sigh', 'nigh', 'thigh', 'bight', 'wight', 'fright', 'bright', 'kn[...]

    'tough', 'rough', 'cough', 'dough', 'though', 'through', 'bough', 'thorough', 'borough', 'trough', 'plough', 'enough', 'ought', 'bought', 'brought', 'caught', 'fought', 'sought', 'taught', 'though[...]

    'would', 'could', 'should', 'mould', 'wouldnt', 'couldnt', 'shouldnt',

    'able', 'cable', 'fable', 'gable', 'label', 'sable', 'table', 'stable', 'capable', 'enable', 'disable', 'reliable', 'variable', 'edible', 'visible', 'divisible', 'terrible', 'horrible', 'incredibl[...]

    'annual', 'manual', 'casual', 'ritual', 'actual', 'usual', 'virtual', 'sensual', 'textual', 'factual', 'sexual', 'mutual', 'gradual', 'individual', 'spiritual', 'dual', 'equal', 'legal', 'regal', [...]

    'easy', 'easel', 'ease', 'please', 'grease', 'tease', 'lease', 'cease', 'increase', 'disease', 'release', 'decrease', 'crease', 'beast', 'feast', 'least', 'yeast', 'east', 'coast', 'roast', 'toast[...]

    'unique', 'unit', 'unite', 'unity', 'unicorn', 'uniform', 'universe', 'university', 'universal', 'unions', 'unison', 'uniquely', 'unify', 'unified', 'unitive', 'united', 'units', 'untie', 'until',[...]

    'scary', 'hairy', 'diary', 'fairy', 'weary', 'dreary', 'canary', 'binary', 'library', 'primary', 'summary', 'contrary', 'military', 'ordinary', 'secretary', 'boundary', 'necessary', 'temporary', '[...]
]
...
async def disconnect(ctx):
    """Leave the current voice channel. Usage: -disconnect"""
    
    # Get the bot's voice client
    voice_client = ctx.guild.voice_client if ctx.guild else None
    
    if not voice_client:
        msg = await ctx.send("not in a voice channel dork")
        store_response(ctx, msg)
        return
    
    if not voice_client.is_connected():
        msg = await ctx.send("not connected to any voice channel")
        store_response(ctx, msg)
        return
    
    try:
        await voice_client.disconnect()
        msg = await ctx.send(f"✅ disconnected from voice channel")
        store_response(ctx, msg)
